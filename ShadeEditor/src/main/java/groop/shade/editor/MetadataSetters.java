...
public class MetadataSetters {
	...
	/**
	 * Edits stage size, prompting user input with a dialog box. Accepts all
	 * strings as long as there are two numbers with some kind of separation
	 * between them. Then checks whether the numbers fall within the range of
	 * (MIN_WIDTH, MIN_HEIGHT) to (MAX_WIDTH, MAX_HEIGHT)
	 * 
	...
	private static Dimension inputSize(String preExisting) {
		...
		try {
			...
			// If user presses cancel or quits, chars == null and
			// NullPointerException occurs.
			int index = 0;
			lbl: for (int listIndex = 0; listIndex < 2; listIndex++) {
			...
			if (widthChars.size() == 0 || heightChars.size() == 0) {
				...
				try {
					...
					if (width > MAX_STAGE_WIDTH || height > MAX_STAGE_HEIGHT || width < MIN_STAGE_WIDTH
	...
	private static Dimension invalidSize(String preExisting) {
		int i = JOptionPane.showConfirmDialog(null,
				"Please enter a valid size smaller than (" + Integer.toString(MAX_STAGE_WIDTH) + ", "
						+ Integer.toString(MAX_STAGE_HEIGHT) + ") and larger than (" + Integer.toString(MIN_STAGE_WIDTH)
						+ ", " + Integer.toString(MIN_STAGE_HEIGHT) + ").",
				"Invalid Size", JOptionPane.OK_CANCEL_OPTION);
