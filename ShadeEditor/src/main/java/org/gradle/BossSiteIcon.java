...
public class BossSiteIcon extends StageIcon implements ChangeableIcon {
	...
	@Override
	protected void setObject(int x, int y) {} // Never called
	...
	protected void setMenu() {
		...
		AttributeSlider attributeSlider2 = new AttributeSlider(-3, 3, 2, "Speed") {
			...
			public void setInitialValue() {
				if (((BossSite)object).bossSpeed > 0) {
					slider.setValue(Math.round((float)(Math.log(((BossSite)object).bossSpeed) / ((Math.log(slider.base)) / (Math.log(5.0))))));
				} else {
					slider.setValue(0);
				}
			}
		...
		for (BossType bossType : BossType.values()) {
			...
			if (bossType == ((BossSite)object).type) button.setSelected(true);
			group.add(button);
			typeMenu.add(button);
		}
		menu.add(typeMenu);
