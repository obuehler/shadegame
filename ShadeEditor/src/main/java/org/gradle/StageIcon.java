...
public abstract class StageIcon extends JLabel {
	...
	public StageIcon(Vector2 o, StageEditor e) {
		preInitializeIcon(Math.round(o.x * Constants.DEFAULT_SCALE),
				e.backgroundPanel.getHeight()-Math.round(
						o.y * Constants.DEFAULT_SCALE), e);
		object = o;
		initializeIcon();
	}
	...
	/** Moves the icon to the (x,y) on the panel and resets the associated object's position. */
	...
	protected Vector2 stagePosition() {
		return new Vector2(xCoordinate,editor.backgroundPanel.getHeight()-yCoordinate).scl(1f/Constants.DEFAULT_SCALE);
	}
	...
	/** Displays the customize menu at the top and produces an outline 
	 * indicating the icon has been selected. */
	public void select() {
	...
	/** Deletes the icon from the background panel and its associated object from the stage. */ 
	...
	/** 
	 * Changes the assigned editor after a drag and drop operation. 
	 * This also includes deleting the associated object from the stage in the
	 * old editor and creating a new one in the stage in the new editor. 
	 * */
