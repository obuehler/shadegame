...
import org.gradle.ObjectSelector.BirdButton;
import org.gradle.ObjectSelector.EraserButton;
import org.gradle.ObjectSelector.SelectorButton;


//import application.GDXRoot;
...
public class StageEditor extends JFrame {
	...
	class BackgroundPanel extends JPanel {
		...
		public BackgroundPanel() {
		    ...
		    dropTarget = new DropTarget(this,
		        DnDConstants.ACTION_MOVE, editorListener);
	 ...
	 * @param newStage = true if creating newStage, false if opening one */
	...
	private void setAttributes() {
		...
		menuPanel.setPreferredSize(new Dimension(200, 2000)); // 2000 is an arbitrary large number that will definitely fill the screen vertically
		...
		AttributeSlider arrowSpeed = new AttributeSlider(-3, 3, 2, "Arrow Speed") {
			...
			public void setInitialValue() {
				if (stage.arrowSpeed > 0) {
					slider.setValue(Math.round((float)(Math.log(stage.arrowSpeed) / ((Math.log(slider.base)) / (Math.log(5.0))))));
				} else {
					slider.setValue(0);
				}
			}
		...
		AttributeSlider rockSpeed = new AttributeSlider(-3, 3, 2, "Rock Speed") {
			...
			public void setInitialValue() {
				if (stage.rockSpeed > 0) {
					slider.setValue(Math.round((float)(Math.log(stage.rockSpeed) / ((Math.log(slider.base)) / (Math.log(5.0))))));
				} else {
					slider.setValue(0);
				}
			}
		...
		AttributeSlider minionSpeed = new AttributeSlider(-3, 3, 2, "Flying Minion Speed") {
			...
			public void setInitialValue() {
				if (stage.minionSpeed > 0) {
					slider.setValue(Math.round((float)(Math.log(stage.minionSpeed) / ((Math.log(slider.base)) / (Math.log(5.0))))));
				} else {
					slider.setValue(0);
				}
			}
		...
		AttributeSlider birdSpeed = new AttributeSlider(-2, 2, 2, "Bird Speed") {
			...
			public void setInitialValue() {
				if (stage.birdSpeed > 0) {
					slider.setValue(Math.round((float)(Math.log(stage.birdSpeed) / ((Math.log(slider.base)) / (Math.log(5.0))))));
				} else {
					slider.setValue(0);
				}
			}
		...
		tutorialButton.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent arg0) {
				if (tutorialButton.isSelected()) stage.setIndex(0);
			}
		...
		scrimmageButton.addChangeListener(new ChangeListener() {
			...
			@Override
			public void stateChanged(ChangeEvent arg0) {
				if (scrimmageButton.isSelected()) stage.setIndex(1);
			}
		...
		battleButton.addChangeListener(new ChangeListener() {
			...
			@Override
			public void stateChanged(ChangeEvent arg0) {
				if (battleButton.isSelected()) stage.setIndex(2);
			}
	...
	private void quitEditor() {
		dispose();
		instancesRunning--;
		if (instancesRunning <= 0) { System.exit(0); } 
	}
	...
	private JButton testButton() {
		...
		button.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				int save = JOptionPane.showConfirmDialog(getThis(), "The stage needs to be saved before testing. Go ahead?");
				if (save == JOptionPane.YES_OPTION) {
					save();
					//new LwjglApplication(new GDXRoot(stage), Constants.STANDARD_CONFIG); THIS NORMALLY STARTS THE GAME
				}
			}
		});
		return button;
	...
	private JButton backgroundButton() {
		...
		button.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				@link http://stackoverflow.com/questions/16303868/jfilechooser-that-will-only-select-java-supported-image-file-formats */
	...
	void editStageSize() {
		...
		if (size != null && stage.pixelSize != size) {
			...
			// We don't remove objects that lie outside of the new size, just so we can reuse them if we want to.
	...
	void exportImage() {
		...
		final BufferedImage image = new BufferedImage(backgroundPanel.getWidth(), backgroundPanel.getHeight(), BufferedImage.TYPE_INT_ARGB);
		backgroundPanel.paint(image.getGraphics());
		JFileChooser chooser = Helpers.chooseFile(this);
		chooser.setAcceptAllFileFilterUsed(false);
		chooser.addChoosableFileFilter(new FileNameExtensionFilter("Portable Network Graphics (*." + Constants.EXPORT_IMAGE_FORMAT + ")", Constants.EXPORT_IMAGE_FORMAT));
	...
	/** Checks if file already exists under approveSelection. The override
	 * code sourced from
	...
	void saveAs() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			if (!savePath.endsWith(Constants.DOTTED_STAGE_FILE_EXTENSION())) { savePath += Constants.DOTTED_STAGE_FILE_EXTENSION(); }
			try {
		...
		SwingWorker<Boolean, Boolean> worker = new SwingWorker<Boolean, Boolean>(){
			...
			protected Boolean doInBackground()  {
				try {
					...
					Files.copy(Paths.get(backgroundPath), Paths.get(stage.fullBackgroundPath()), StandardCopyOption.REPLACE_EXISTING);
					returnValue = true;
				} catch (IOException e) {
					returnValue = false;
				}
	...
	private void setKeyboardShortcuts() {
		...
		getRootPane().getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_Z, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()), "undo");
		getRootPane().getActionMap().put("undo", EditorActions.undoAction(this));
		getRootPane().getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_Y, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()), "redo");
		getRootPane().getActionMap().put("redo", EditorActions.redoAction(this));
		getRootPane().getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_N, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()), "new");
		getRootPane().getActionMap().put("new", EditorActions.newAction());
		getRootPane().getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_O, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()), "open");
		getRootPane().getActionMap().put("open", EditorActions.openAction(this));
		getRootPane().getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_S, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()), "save");
