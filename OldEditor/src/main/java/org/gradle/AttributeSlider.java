...
public abstract class AttributeSlider extends JPanel implements ChangeListener {
	...
	public static class CustomSlider extends JSlider implements ChangeListener {
		...
		public CustomSlider(int min, int max, int b) {
			...
			for (int k = min; k <= max; k++) {
				if (k < 0) {
					labelTable.put(new Integer(k * MINOR_TICKS_PER_MAJOR_TICK), new JLabel("1/" + (int)Math.pow(base, -k)));
				} else {
					labelTable.put(new Integer(k * MINOR_TICKS_PER_MAJOR_TICK), new JLabel(Integer.toString((int)Math.pow(base, k))));
				}
			}
	...
	public void refreshText() {
		if (slider.base == null) {
			...
			valueLabel.setText(Double.toString(Math.round(Math.pow(slider.base, (double)slider.getValue() / MINOR_TICKS_PER_MAJOR_TICK) * 100.0) / 100.0));
		}
	}
